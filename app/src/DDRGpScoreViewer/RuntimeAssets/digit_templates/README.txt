...
* ROI-specific directory (`score_digits` or `miss`)
...
The packaged template sets are `score_digits`, `judgment_counts`,
`combo_ex_score`, and `miss`. The runtime still accepts additional
ROI-specific or fallback directories from an explicit runtime data path.
