...
M5c のローカル jacket catalog 運用を支援する、公開 `DDRGpScoreViewer` とは独立した Windows WPF app です。master DB の明示更新、coverage/review projection、catalog v1からv2への非破壊移行、v2からv3へのCLI copy-on-write backfill、監査可能なmanual reviewに加え、DDR GRAND PRIX window候補の確認とmemory-only capture lifecycle観測を扱います。通常の公開app build、installer、Release、GitHub Actions artifactには含めません。
...
UIはcatalog SQLite tableを直接読みません。Python projectionがmaster/catalogをstrictかつread-onlyで検証し、UTF-8 stdoutへversion 2の単一JSONを出します。catalog v1はmigration-required/read-only、v2/v3はmanual-review capableとして投影します。temporary projection fileは作りません。stderrは診断専用です。C# loaderは既存version 1 fixtureのread-only表示互換を維持します。
...
v2→v3は現在UIから暗黙実行せず、次の明示CLIで新規targetへだけ行います。artifact root内の一意なmanifest、`source.png`、`jacket-crop.png`を検証できたrowだけcomposite identityをbackfillし、欠損・改変・unknown version・複数locator・identity競合はreceiptの未適用statusへ残します。source catalog、artifact、既存targetは変更しません。
...
capture tabへ既存session IDを入力して `session再開` を押すと、current projection・選択window・全artifact manifest/hashが一致するcompatible checkpointだけを再開します。既存`m5c-observation-manifest-v1` / `m5c-observation-checkpoint-v1`は変更せず、legacy resume/retryはv1のまま継続します。v1とv2を混在させるresume/retryは拒否します。catalog v1にはtitle/artistを推測せず空文字と `unresolved` で既存M5b ingest APIを呼び、catalog v2には従来のversioned ingest、catalog v3には同じ入口からmanifest v2のjacket/title/composite identity一式を必須で渡します。v3 writerはtransaction内で全review status（`rejected`を含む）の同一identityを既存referenceへ収束させ、2件目を作りません。旧M5c-3bで `deferred` になったv2 catalog observationとcatalog failure後の `pending` は、`catalog retry (pending/deferred)` から既存writerへ明示再投入できます。
...
## v2/v3 unresolved observation ingest (M5c-3c)

Pythonの`ingest-v2`はv1の`ingest`とは別のversion-aware CLIです。catalog schema 2/3、catalog identity/created-at、current master version/source hash、current feature extractor、artifact image hashをstrictに検査し、schema変更やv1 writerの暗黙変更は行いません。v3では既知versionのjacket/title-line/composite identityとcanonical hash一致を追加で必須にします。新規rowは`source_capture_id=observation_id`、`song_id=NULL`、空title/artist、`review_status=unresolved`、`review_revision=0`、manual action/noteなし、candidate/historyなしで作成されます。
...
`title/artist評価`は、collectorで明示採用済みのimmutable `observation.json` / `source.png` / `jacket-crop.png`だけを読むdeveloper-only評価入口です。catalog v2/v3、current master version/source hash、catalog identity/created-at、current feature extractor、manifest/image hash、artifact root内相対pathをstrictに検査し、catalog、checkpoint、manual review revision/historyを変更しません。v2→v3 migrationはcreated-atを維持するため、v3 catalogでは元v2 schema identityを持つartifactも評価入力として許容します。
