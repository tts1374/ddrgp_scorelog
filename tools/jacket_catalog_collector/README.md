...
候補pathは毎回未作成の名前を使用します。`import-result-text-features`はbinding済みruntime catalogから未bindingのcollector sourceへM7 result-text featureだけを取り込みます。runtime catalog全体をsourceへcopyせず、以後の候補はsourceから`bind-master`で一方向に生成します。`bind-master`は既存runtime catalogをin-place更新せず、候補生成と`release-pair`成功後にだけ利用先を切り替えます。Releaseへ進む場合は、さらに[`app/README.md`](../../app/README.md)のreference data set packagingと公開前検証に従います。
...
同じcapture frameから、1280x720基準の`INFORMATION`見出し `(286,35,134,23)` と曲名行 `(286,64,504,25)` も観測します。各ROIを基準sizeへ正規化し、RGB各channel `>=170`かつchannel差`<=45`の白系文字をbinary maskへ変換します。見出しの白画素数100以上、見出し内の横方向の白帯の最大行長120以上、曲名行32 pixel以上を同時に満たした場合だけ表示ありとします。これは文字OCRではなく、`INFORMATION`パネル固有の画面形状を含めた視覚ゲートです。曲名行maskのSHA-256が連続3 frame・最小100ms一致した場合だけ安定と表示します。detector、ROI、feature方式はいずれもversion付きで診断表示します。
