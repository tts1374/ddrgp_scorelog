...
`m7_save_readiness_review.csv`、`m7_save_readiness_review.json`、`m7_save_readiness_review.md` は、M3保存候補材料とM7a数字材料を confirmed-events 1件単位で束ねるM7保存判定前レビューです。入力は `m3_save_candidate_summary_rows` と `m7a_digit_save_candidate_summary_rows` に限定し、`ready_for_save_review`、`blocked_m3_material`、`blocked_digit_review`、`missing_required_material` を出します。`ready_for_save_review` はレビュー材料が揃った状態であり、保存OK、DB保存成功、曲ID/譜面ID確定を意味しません。duplicate、`rejected_transition`、未確定候補、non-result は対象外のままです。
