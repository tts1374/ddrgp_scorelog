...
`m8_planned_play_records.csv`、`m8_planned_play_records.json`、`m8_planned_play_records.md` は、M8 payload previewの `payload_ready` 行だけを個人スコアDB `plays` 相当の最小row contractへ変換するプレビューです。`unsupported_preview_status`、`missing_identity_candidate`、`missing_digit_value` は保存予定レコードへ変換しません。最小SQLiteスキーマはテストの in-memory fixtureで確認し、実DBファイル生成やDB insertには進みません。`song_id` / `chart_id` はM5候補観測、数字列はM7a候補値であり、保存用確定IDや保存値確定ではありません。
...
- M8 planned play recordsは `payload_ready` だけを `plays` 最小row contractへ変換し、非ready payloadを保存予定レコードへ進めず、in-memory SQLite fixtureでスキーマ契約を固定できる
