...
正式schema contractの検査入口は `inspect_personal_score_db_schema()` と `assert_personal_score_db_compatible()` です。検査では空DBを初期化候補、M8 preview DBを `reject_m8_preview_database`、metadata欠損やidentity不一致を `reject_unknown_database`、正式metadataつきのversion/table不整合を `manual_migration_required` として分けます。これは本番insertや自動migrationではなく、既存DBを正式個人スコアDBとして開いてよいかを説明するための足場です。
