...
- M8 score DB file output previewは `--m8-score-db-output` 明示時だけ `data/` 配下の新規SQLiteファイルへ保存予定レコードをinsertし、DBの `PRAGMA user_version=1`、`preview_metadata.created_by_preview`、summary/reportの `schema_version=1` と `created_by_preview` を確認でき、本番保存成功として扱わない
