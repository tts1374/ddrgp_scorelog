...
低信頼度/error再調査用のpure contractは `tools.vision_poc.personal_score_db_analysis_artifacts` に分けています。version 1詳細JSONはstrict validationでunknown key、正式play値、validation receipt key、DB diagnostic payloadを拒否します。`analysis_logs.log_path` は空文字またはrepository root基準の `logs/analysis_details/**/*.json`、任意の失敗画像は別fieldで `logs/analysis_failures/` 配下だけを参照します。retentionは `short=7日`、`standard=30日`、`indefinite=期限なし` をUTC基準で計算します。このmoduleはJSONや画像を生成・削除せず、save CLIへ自動連鎖しません。
