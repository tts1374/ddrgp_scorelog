...
artifactと正式saveの接続は、既存CLIを順番に実行する暗黙workflowではなく、後続PRの単発明示orchestration入口で実装します。入口は入力/adapter、共有ID/status/path、DB互換性と早期duplicateを検査してからartifactを生成し、最後に既存file saveを1回呼びます。低信頼度/errorはartifact必須、ready・その他skip・DB duplicateは任意です。入口がoutput pathと `analysis_logs.log_path` の一致を保証し、正式play値・candidate material・analysis detailを相互補完しません。artifact後のDB失敗ではartifactを残し、同一payloadだけ再利用します。現時点のCLIは自動接続せず、終了コードも変わりません。
