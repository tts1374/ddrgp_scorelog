...
migration可否は専用のread-only CLIで確認できます。`python -m tools.vision_poc --personal-score-db-migration-status path\to\ddrgp-scores.sqlite --personal-score-db-migration-target-version 1 --personal-score-db-migration-backup path\to\new-backup.sqlite` は、schema inspection、formal identity、`PRAGMA user_version`、metadata version、連続したmigration履歴、明示backup pathの存在・source同一性・親directoryを検査し、pure migration contractのstatus/reason/source/target/予定step/終了コードをMarkdownで表示します。`--personal-score-db-migration-dry-run` は同じcontractをdry-run入力で評価し、`--personal-score-db-migration-format json` は同じprojectionをJSONで表示します。これらのoptionは専用group以外と排他で、DB、backup、`data/`、`logs/`を作成・変更せず、migration、backup、repair、save/orchestration/diagnosticを実行しません。preview、unknown、identity mismatch、newer unsupported、version/history不一致のpartial stateは正式DBへ昇格しません。

検証済みbackupは `python -m tools.vision_poc --personal-score-db-backup-source path\to\ddrgp-scores.sqlite --personal-score-db-backup-output path\to\new-backup.sqlite` で明示的に1件だけ作成できます。sourceはcompatibleな現行正式DBに限定し、backup先は既存ファイルを上書きしないexclusive createです。SQLite backup APIによる同一source snapshotのコピー、close後flush、read-only再open、`PRAGMA integrity_check`、formal identity/version/migration history/必須table row countの一致を全て通過した場合だけ `verified` を返します。失敗時は今回作成した不完全backupだけを除去し、sourceと既存backupは変更しません。`--personal-score-db-backup-format json` で機械可読結果を表示できます。この専用CLIはmigration、restore、repair、retention、save/orchestration/diagnosticからの自動実行には接続しません。

正式保存入力とconnection単位の最小transaction writerは `tools.vision_poc.personal_score_db_save` に分けています。`PersonalScoreDbSaveInput` はM8 preview payloadを直接受け取らず、timezone付き `played_at` / `captured_at`、`master_version`、確定済みID/数字、空でない `rank` / `clear_type`、正式duplicate key、confidence、app versionを要求します。

低信頼度/error再調査用contractと明示file outputは `tools.vision_poc.personal_score_db_analysis_artifacts` に分けています。version 1詳細JSONはstrict validationでunknown key、正式play値、validation receipt key、DB diagnostic payloadを拒否します。`write_analysis_detail_file()` は検査済みpayloadをrepository root基準の新規 `logs/analysis_details/**/*.json` へ、UTF-8 BOMなし、LF、決定的key順、末尾改行でatomicに1件だけ生成します。CLIでは `python -m tools.vision_poc --personal-score-db-analysis-detail-input path/to/detail.json --personal-score-db-analysis-detail-output logs/analysis_details/manual/detail.json` の必須ペアを使います。任意の失敗画像は `logs/analysis_failures/` のpathを検査するだけで生成・copyせず、DB、`data/`、save CLI、通常PoCへ自動連鎖しません。

artifactと正式saveの接続は `tools.vision_poc.personal_score_db_workflow` の単発明示orchestration入口で実装しています。version 1 workflow JSONは `analysis_detail` と `save_input` を別objectで保持し、`--personal-score-db-workflow-input`、任意の `--personal-score-db-workflow-artifact-output`、必須の `--personal-score-db-workflow-database` だけで実行します。入力/adapter、共有ID/status/path、既存DB互換性と早期duplicate、artifact publish/reuse、既存file save 1回の順で進みます。低信頼度/errorはartifact必須、ready・その他skip・DB duplicateは任意です。artifact失敗時はDBを呼ばず、artifact後のDB失敗ではartifactを保持し、同一payloadの既存artifactだけを再利用します。結果はworkflow/artifact/adapter/DB status、ID、理由、pathだけを返し、終了コードは完了0、`unresolved` 1、invalid・DB/artifact失敗2です。既存artifact/save/validation/template/diagnostic CLIの独立性と終了コードは変えません。
...
新規catalogは repository root直下の `databases/` 配下の未作成pathへ明示作成します。capture artifact、report、coverage outputは従来どおり `data/` 配下に限定します。
...
初期版Release前に作成済みで`catalog_metadata.master_version`を持たないcurrent catalogは、sourceを変更せず新規outputへcopyしてcurrent masterへ明示bindingします。bindingはcatalog全tableをcopyし、旧masterのjacket referenceと`result_text_features`を削除・上書きしません。runtimeは元の`master_version`を履歴として保持したまま、current GP masterのsong ID・canonical title・canonical artistが一致するhistorical master-compatible featureをread-onlyで再利用します。binding済みruntimeにだけM7 result-text featureが残っている場合は、`import-result-text-features --source-catalog <runtime> --target-catalog <collector-source>`でfeatureだけをcollector sourceへ冪等に取り込み、targetの`master_version`設定有無は変更しません。reference、review履歴、catalog metadataは逆同期せず、その後のruntime候補はsourceから`bind-master`で生成します。出力後にsourceとoutputを取り違えないようpathを確認し、Debug WPFのdevelopment runtimeとRelease packageにはbinding済みoutputだけを渡します。
