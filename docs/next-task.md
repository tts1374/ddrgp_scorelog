...
- M7a `max_combo`、`marvelous`、`perfect` は、ROI左側ラベルや下線を数字扱いせず、右側数字領域へ寄せたcomponent分割を維持している。
- M7a `great` を非OCR認識対象へ広げた。
  - ROI左側ラベルを数字扱いしないよう、右側数字領域へ寄せて前景コンポーネントを分割する。
  - 3桁fixture、4桁fixture、テンプレート不足時の `missing_reference` + 分割数診断、共有 `judgment_counts` テンプレートfallbackを追加した。
  - `great` のfixtureはネットワーク、ローカル画像、`metadata.csv` に依存しない。
- M7a digit template の共有探索は維持している。
  - ROI別ディレクトリを最優先する。
  - `marvelous`、`perfect`、`great`、今後の `good`、`miss` は `digit_templates/judgment_counts/` を共有候補として読む。
...
- 2026-07-07のローカル `score_digits + max_combo + marvelous + perfect + great` 実行結果:
  ...
  - `match_count=300`
  - `mismatch_count=0`
  - `great`: 60/60 match、`segment_count_counts={"1":22,"2":32,"3":6}`、`expected_digit_length_counts={"1":22,"2":32,"3":6}`
- 2026-07-07のTesseract比較あり実行結果:
  ...
  - M7aは `score_digits + max_combo + marvelous + perfect + great` 合計300/300 match。
  - `tesseract_comparison.available_attempts=59`
  - `same_normalized_count=56`
  - `different_normalized_count=3`
  - `unavailable_count=241`
  - unavailable は既定OCR対象が `score_digits` のため、`max_combo`、`marvelous`、`perfect`、`great` の各60件と、OCR未取得の `score_digits` 1件が比較対象なしになる。
- 更新済み:
...
M7aの次ステップとして、`score_digits`、`max_combo`、`marvelous`、`perfect`、`great` の可変桁・右寄せ分割、4桁fixture、共有テンプレートfallback、300/300 matchを保ったまま、判定数ROIを小さい範囲から広げる。

- 次は `good` を第一候補にする。必要に応じて `miss` の順で広げる。
- `good`、`miss` はまず `judgment_counts` 共有テンプレートで認識できるか確認し、必要な場合だけROI別テンプレートを追加する。
...
- `max_combo`、`marvelous`、`perfect`、`great` の右側数字領域分割を壊さない。
- ローカルテンプレートあり環境では `score_digits + max_combo + marvelous + perfect + great` の300/300 matchを維持する。
- `max_combo`、`marvelous`、`perfect`、`great` の4桁fixtureを壊さない。
...
- `score_digits + max_combo + marvelous + perfect + great` の300/300 matchを壊していない。
- `max_combo`、`marvelous`、`perfect`、`great` の4桁fixtureを壊していない。
- 共有 `judgment_counts` テンプレートだけで `marvelous` / `perfect` / `great` を読めるfixtureを壊していない。
