...
最新確認コミットは、このファイルを含む `Add M3 chart-field inventory report` コミットです。このコミットまでで、M3 chart-field 評価の足場として `m3_chart_fields.csv` と `m3_chart_fields_summary.json` が追加されています。
...
- 最新コミットが `Add M3 chart-field inventory report` 以降であること
...
- duplicate、`event_type=rejected_transition`、未確定 `result_candidate`、non-result は M3 metadata expected coverage 対象外。
- `m3_chart_fields.csv` を追加した。全イベント行を出し、`chart_field_target=true` は confirmed-events 境界の `confirmed_result=true` かつ `duplicate=false` のみ。
- `m3_chart_fields.csv` の初期対象fieldは `play_style`、`difficulty`、`level` のみ。`rank`、`song_title`、`artist` は初期 chart-field 評価対象に含めない。
- `m3_chart_fields.csv` は `expected_play_style`、`expected_difficulty`、`expected_level` と `rois/<画像名>/<field>.png` の相対パスを出す。抽出成功やマスタ照合成功を意味しない。
- 対象外イベントは `exclusion_reason` で `duplicate`、`rejected_transition`、`unconfirmed`、`non_result` として区別する。
- `m3_chart_fields_summary.json` は `target_boundary`、`chart_field_target_count`、`excluded_counts`、`fields` を出す。
- `tools/vision_poc/README.md`、`docs/design/03_event_and_save_boundary.md`、`docs/design/06_regression_guard.md` に M3 chart-field 足場の読み方を追記済み。
- `tests/test_vision_poc_ocr.py` で、chart-field 足場が confirmed-events 境界だけを対象にし、duplicate / rejected_transition / unconfirmed を除外することを確認している。
...
- `m3_chart_fields_summary.json` の `play_style` / `difficulty` / `level` は、confirmed-events 対象60件で60/60 `evaluated`。
...
1. 現状確認
   ...
   - `data/vision_poc/m3_chart_fields.csv` と `data/vision_poc/m3_chart_fields_summary.json` を読み、chart-field 対象が confirmed-events 60件だけであること、対象外理由が `duplicate` / `rejected_transition` / `unconfirmed` / `non_result` に分かれていることを確認する。

2. M3 chart-field 抽出PoCの次の最小単位を決める
   - 次の実装対象は `play_style`、`difficulty`、`level` の抽出評価に限定する。
   ...
   - 抽出結果を出す場合は、既存の `score_ocr.csv`、`score_ocr_summary.json`、`ocr_expected_coverage.md`、`m3_metadata_expected_coverage.md`、`m3_chart_fields.csv` の意味を変えず、別CSV/JSONに分ける。

3. 軽い chart-field 抽出評価の候補
   - まずは `play_style`、`difficulty`、`level` の期待値とROI画像パスがそろっている confirmed-events だけを対象にする。
   - 実装する場合は、抽出値、期待値、match、status、failure_reason を持つ新しいCSV/JSONを検討する。
   - テンプレート比較や色特徴を試す場合も、ローカル画像に依存しない単体テストで境界や集計を先に固定する。
...
- duplicate / rejected_transition / unconfirmed / non-result が保存直前OCR評価対象外、M3 metadata expected coverage 対象外、M3 chart-field 対象外のまま。
