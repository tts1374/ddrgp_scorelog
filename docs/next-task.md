...
- M7a `max_combo` を非OCR認識対象へ広げた。
  - ROI左側ラベルや下線を数字扱いしないよう、右側数字領域へ寄せて前景コンポーネントを分割する。
  - テンプレート不足時でも `segment_count_counts` と `expected_digit_length_counts` をsummary/reportに出し、分割数と期待桁数を確認できる。
  - ネットワーク、ローカル画像、`metadata.csv` に依存しないfixtureで `max_combo=198` の認識と、テンプレート不足時の `missing_reference` + `segment_count=3` を固定した。
- ローカル素材として `samples/screenshots/organized/digit_templates/max_combo/0.png` から `9.png` を生成した。これはGit管理しない。
- 2026-07-07のローカルM7a実行結果:
  ...
  - `score_digits`: 60/60 `recognized` / match。
- 2026-07-07のローカル `score_digits + max_combo` 実行結果:
  - `python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo --no-ocr --no-rois --output data\vision_poc_m7a_digit_max_combo_templates`
  - 分類: 221/221 correct、false positives 0、false negatives 0。
  - `target_count=60`
  - `total_attempts=120`
  - `status_counts={"recognized": 120}`
  - `match_count=120`
  - `mismatch_count=0`
...
- 2026-07-07のTesseract比較あり実行結果:
  ...
  - M7aは `score_digits + max_combo` 合計120/120 match。
  - `tesseract_comparison.available_attempts=59`
  - `same_normalized_count=56`
  - `different_normalized_count=3`
  - `unavailable_count=61`
  - unavailable は既定OCR対象が `score_digits` のため、`max_combo` 60件とOCR未取得の `score_digits` 1件が比較対象なしになる。
- 更新済み:
...
M7aの次ステップとして、`score_digits` と `max_combo` の可変桁・右寄せ分割と120/120 matchを保ったまま、判定数ROIを小さい範囲から広げる。

- 次は `marvelous` を第一候補にする。必要に応じて `perfect`、`great`、`good`、`miss` の順で広げる。
