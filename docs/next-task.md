...
- M7a `marvelous` は、ROI左側ラベルを数字扱いせず、右側数字領域へ寄せたcomponent分割を維持している。
- M7a `perfect` を非OCR認識対象へ広げた。
  ...
  - ネットワーク、ローカル画像、`metadata.csv` に依存しないfixtureで `perfect=128` の認識と、テンプレート不足時の `missing_reference` + `segment_count=3` を固定した。
- レビュー対応として、`max_combo`、`marvelous`、`perfect` の4桁fixtureを追加し、3桁だけでなく4桁でも右側数字領域を分割・認識できることを固定した。
- M7a digit template の共有探索を追加した。
  - ROI別ディレクトリを最優先する。
  - `marvelous`、`perfect`、今後の `great`、`good`、`miss` は `digit_templates/judgment_counts/` を共有候補として読む。
  - `max_combo` と今後の `ex_score` は `digit_templates/combo_ex_score/` を共有候補として読む。
  - ROI別テンプレートを残したまま共有ディレクトリを試せるため、ローカル素材の削除・移動なしで共通化可否を検証できる。
- ローカル素材として `samples/screenshots/organized/digit_templates/perfect/0.png` から `9.png` を配置した。これはGit管理しない。
- ローカル素材として `samples/screenshots/organized/digit_templates/judgment_counts/0.png` から `9.png` を配置した。これはGit管理しない。
- 2026-07-07のローカル `score_digits + max_combo + marvelous + perfect` 実行結果:
  ...
  - `perfect`: 60/60 match、`segment_count_counts={"1":1,"2":42,"3":17}`、`expected_digit_length_counts={"1":1,"2":42,"3":17}`
- 2026-07-07の共有 `judgment_counts` テンプレート確認:
  - `python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo marvelous perfect --m7a-digit-template-root data\m7a_shared_judgment_template_trial --no-ocr --no-rois --output data\vision_poc_m7a_digit_shared_judgment`
  - `target_count=60`
  - `total_attempts=240`
  - `status_counts={"recognized":240}`
  - `match_count=240`
  - `mismatch_count=0`
  - この試験rootでは `marvelous` と `perfect` のROI別テンプレートを置かず、共有 `judgment_counts` だけで読めることを確認した。
- 2026-07-07の `perfect` テンプレート不足確認:
  - `python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo marvelous perfect --m7a-digit-template-root data\m7a_no_perfect_template_trial --no-ocr --no-rois --output data\vision_poc_m7a_digit_perfect_missing_reference`
  - `perfect`: テンプレート配置前は 60/60 `missing_reference`
  - `segment_count_counts={"1":1,"2":42,"3":17}`
  - `expected_digit_length_counts={"1":1,"2":42,"3":17}`
- 2026-07-07のTesseract比較あり実行結果:
  ...
  - M7aは `score_digits + max_combo + marvelous + perfect` 合計240/240 match。
  - `tesseract_comparison.available_attempts=59`
  - `same_normalized_count=56`
  - `different_normalized_count=3`
  - `unavailable_count=181`
  - unavailable は既定OCR対象が `score_digits` のため、`max_combo` 60件、`marvelous` 60件、`perfect` 60件、OCR未取得の `score_digits` 1件が比較対象なしになる。
- 更新済み:
...
M7aの次ステップとして、`score_digits`、`max_combo`、`marvelous`、`perfect` の可変桁・右寄せ分割、4桁fixture、共有テンプレートfallback、240/240 matchを保ったまま、判定数ROIを小さい範囲から広げる。

- 次は `great` を第一候補にする。必要に応じて `good`、`miss` の順で広げる。
- `great`、`good`、`miss` はまず `judgment_counts` 共有テンプレートで認識できるか確認し、必要な場合だけROI別テンプレートを追加する。
- `ex_score` は桁見た目やTesseract profile差分が別物なので、判定数ROIの状況を見てから扱う。ただし `max_combo` とフォントデザインが近い可能性があるため、M7aで扱うときは `combo_ex_score` 共有テンプレートを第一候補として検証する。
...
- `max_combo`、`marvelous`、`perfect` の右側数字領域分割を壊さない。ローカルテンプレートあり環境では `score_digits + max_combo + marvelous + perfect` の240/240 matchを維持する。
- `max_combo`、`marvelous`、`perfect` の4桁fixtureを壊さない。
...
python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo marvelous perfect --m7a-digit-template-root data\m7a_no_perfect_template_trial --no-ocr --no-rois --output data\vision_poc_m7a_digit_perfect_missing_reference
python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo marvelous perfect --m7a-digit-template-root data\m7a_perfect_template_trial --no-ocr --no-rois --output data\vision_poc_m7a_digit_perfect_template_trial
python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo marvelous perfect --no-ocr --no-rois --output data\vision_poc_m7a_digit_perfect_templates
python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo marvelous perfect --m7a-digit-template-root data\m7a_shared_judgment_template_trial --no-ocr --no-rois --output data\vision_poc_m7a_digit_shared_judgment
python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo marvelous perfect --no-ocr --no-rois --output data\vision_poc_m7a_digit_perfect_review
...
python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo marvelous perfect --ocr-target confirmed-events --no-rois --output data\vision_poc_m7a_digit_perfect_review_ocr_compare
...
python -m tools.vision_poc --m7a-digit-recognition --m7a-digit-rois score_digits max_combo marvelous perfect great --ocr-target confirmed-events --no-rois --output data\vision_poc_m7a_digit_great_ocr_compare
python -m tools.vision_poc --no-ocr
python -m ruff check master tools\vision_poc pyproject.toml tests
python -m compileall master tools\vision_poc
python -m pytest tests
...
- `max_combo`、`marvelous`、`perfect` の4桁fixtureを壊していない。
- 共有 `judgment_counts` テンプレートだけで `marvelous` / `perfect` を読めるfixtureを壊していない。
