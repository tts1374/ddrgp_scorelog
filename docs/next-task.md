...
- M5 jacket通常候補CSV/診断CSVに、ローカルmetadata期待曲名をM4 `songs.title` へ突き合わせた診断列を追加した。
...
- `jacket_match_summary.json` / `jacket_match_diagnostics_summary.json` に期待曲解決状態の集計を追加した。
  - 通常候補60件: `expected_song_resolution_status_counts={"resolved": 58, "unresolved": 2}`
  - 通常候補60件: `expected_song_resolution_reason_counts={"title_not_found": 2}`
  - 通常候補60件: `expected_song_grand_prix_play_available_counts={"True": 58}`
  - 診断118件: `expected_song_resolution_status_counts={"resolved": 104, "unresolved": 14}`
  - 診断118件: `expected_song_resolution_reason_counts={"missing_label": 12, "title_not_found": 2}`
- `jacket_match_report.md` と `jacket_match_diagnostics.md` に `Unresolved Identity Signal Representatives` を追加した。
  - `unresolved_*` だけを抜き出し、期待曲解決状態、GP可否、公式突合状態、jacket状態、expected rank、margin、line-hash辞書状態、top candidate、reasonを同じ表で確認できる。
  - この表は観察補助であり、保存候補化、曲ID確定、`jacket_match_status` 昇格、GP対象外曲の復帰には使わない。
- 通常候補の未解決2件は、今回の追加列で読み分けられる状態になった。
  - `Inner Spirit -GIGA HiTECH MIX-`: `expected_song_resolution_status=resolved` / `expected_song_grand_prix_play_available=True` / `official_availability_match=title_artist`。M4期待曲は解決済みだが、jacket候補がLondon EVOLVED群へ寄り、`title_linehash_dict_status=no_dict_match` のまま `unresolved_ambiguous`。
  - `RЁVOLUTIФN`: `expected_song_resolution_status=unresolved` / `expected_song_resolution_reason=title_not_found`。M4期待曲名へ解決できておらず、`jacket_match_status=not_found` / `failure_reason=above_distance_threshold`。
- 診断側ではLondon EVOLVED ver.Bも未解決代表として読める。
  - `result_254` は期待 `London EVOLVED ver.B` に対して `identity_signal_title=London EVOLVED ver.A` の `composite_resolved_candidate`。
  - `result_255` は `title_linehash_dict_status=no_dict_match` / `unresolved_ambiguous`。
  - どちらも `m5_target_boundary_reason=duplicate` で保存候補外。
- New York EVOLVED Type A/B/C、osaka EVOLVED、tokyoEVOLVED は診断CSVで別行として観察できる状態を維持した。line-hash辞書で期待Typeへ寄る行があるが、duplicate / unconfirmed は保存候補外のまま。
- M5 jacket通常候補結果は60件で、従来境界を維持している。
...
- 2026-07-07の検証では、M4 DBは 1282 songs / 9594 charts / source_snapshots 2件。
  - Wiki source hash は `4491c50700176c6ef647da91004e4f6536b1e2754cdac62b0dff0ebd6bffed53`。
  - 公式収録曲一覧 source hash は `a167ed4f1c2c992abac86a39c194d7ab8b735f7348e5d04a3421a0d5f43c9ba7`。
  ...
  - `official_availability_match` 内訳は `title_artist=1143`、`unique_title=37`、`not_found=102`。
...
- まず今回追加した `Unresolved Identity Signal Representatives` とCSVの `expected_song_*` 列を入口に、通常候補60件と診断118件を混同せずに読む。
  - 通常候補の `Inner Spirit -GIGA HiTECH MIX-` はM4期待曲解決済み/GP対象なので、次に見るべきはjacket候補集合とline-hash参照状態。
  - 通常候補の `RЁVOLUTIФN` はM4期待曲名が `title_not_found` なので、次に見るべきはM4側の曲名表記差・alias/公式可否突合補助。保存候補化やしきい値変更へ直結しない。
  - 診断側のLondon EVOLVED ver.B `result_254` / `result_255` は保存候補外の観察対象として扱う。
- 次に、観察結果に応じて `expected_song_*` や unresolved レポートをさらに扱いやすくする実装を1つ以上進める。
  - 例: M4曲名表記差を観察するalias候補/正規化候補レポート、`expected_song_resolution_reason=title_not_found` 専用代表、line-hash辞書不一致の参照元代表、unresolvedだけのCSV出力、M4 `official_availability_match` の詳細表示など。
  - 実装する場合も、通常候補と duplicate / unconfirmed 診断候補は混ぜない。
  - `expected_song_*` はローカルmetadata期待値の診断列であり、保存判定や候補昇格には使わない。
- M5語彙は維持する。
...
Select-String -Path data\master_match_poc_jacket\jacket_match_report.md -Pattern "Identity Signal Representatives|Unresolved Identity Signal Representatives" -Context 0,8
Select-String -Path data\master_match_poc_jacket\jacket_match_diagnostics.md -Pattern "Boundary Representatives|Identity Signal Representatives|Unresolved Identity Signal Representatives" -Context 0,12
...
Import-Csv data\master_match_poc_jacket\jacket_match_candidates.csv |
  ...
  Select-Object organized_file,expected_song_title,expected_song_id,expected_song_resolution_status,expected_song_resolution_reason,expected_song_grand_prix_play_available,expected_song_official_availability_match,jacket_match_status,failure_reason,expected_jacket_rank,jacket_top_margin,identity_signal_status,identity_signal_source,title_linehash_dict_status,title_linehash_dict_top_title,title_linehash_rerank_reason,top_candidates |
  Format-List
```
...
Import-Csv data\master_match_poc_jacket\jacket_match_diagnostics.csv |
  ...
  Select-Object organized_file,m5_target_boundary_reason,event_type,confirmed_result,duplicate,expected_song_title,expected_song_resolution_status,expected_song_grand_prix_play_available,expected_song_official_availability_match,jacket_match_status,identity_signal_status,identity_signal_source,identity_signal_title,title_linehash_dict_status,title_linehash_dict_top_title |
  Format-List
```
...
- `Unresolved Identity Signal Representatives` で通常候補の `Inner Spirit -GIGA HiTECH MIX-` と `RЁVOLUTIФN` を混同せずに読める。
