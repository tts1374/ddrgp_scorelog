...
M8 previewは完了済みとして扱います。今回、正式M8本体の最初の成果物として、正式個人スコアDBの初期schema contractとmigration/互換チェック境界を追加しました。
...
- M8 preview DBは `user_version=1` でも正式DBとして拒否する。
...
次は正式DBへ書く前のmigration/互換チェックをもう一段進めてください。本番insertはまだ実装しないでください。

第一候補:

- `personal_score_db_compatibility_errors()` のテストを増やし、unknown DB、metadata mismatch、必須table欠落、`user_version` mismatch の拒否理由を固定する。
- 正式DB初期化/検査用の軽い関数を追加する。例: `inspect_personal_score_db_schema()`、`assert_personal_score_db_compatible()`、または migration plan を返す関数。
- `docs/design/10_personal_score_db_schema.md` に、拒否理由語彙と既存DBをどう扱うかを追記する。

第二候補:

- `analysis_logs` と `source_captures` の責務をさらにテストで固定し、保存成功/保存スキップ/低信頼度をどの列で表すかを明確にする。
- ただし、低信頼度ログの本番保存処理や画像保存処理には進まない。

このフェーズで決めたい候補:

- preview DBを見つけた場合のエラー語彙。
- metadata欠損DB、未知schema、古い正式schemaを拒否するかmigration候補として扱うか。
- migrationを自動実行する前に必要なbackup/明示確認の境界。
...
- 正式個人スコアDBのmigration/互換チェック、解析ログ境界、またはsource capture参照境界が1つ以上進んでいる。
