...
- `score_digits` の桁分割を、固定6桁ではなく大きな数字成分を左から読む方式へ調整した。
  - カンマや背景ノイズを数字として数えない。
  - 1桁から7桁までの可変桁表示をfixtureで固定した。
  - 0から1,000,000までの可変桁表示を前提にする。
- ローカル実素材 `samples/screenshots/organized/chart_field_templates/chart_field_template_129_double_challenge_lv19.png` の `score_digits` ROI は3桁表示 `710` で、現行M7a分割は `segment_count=3`、ローカル `score_digits` テンプレート比較では `recognized_digits=710` / `match=True`。
...
- fixtureテストで、M7a confirmed-events境界、missing reference、failed segmentation、not evaluated、カンマ無視、1桁から7桁までの可変桁score_digitsを固定した。
