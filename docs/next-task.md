...
M8 previewは完了済みとして扱います。正式個人スコアDB本体では、schema contract、互換チェック、空DB初期化境界、connection/file path単位のwrite前準備境界、diagnostic dict / Markdown投影、CLI標準出力入口、`data/` 配下へのdiagnostic file output、`logs/` 配下へのdiagnostic JSONL log output入口まで進んでいます。本番insert、既定自動保存、既存DB migration実行、低信頼度ログ本番保存、source capture保存にはまだ進んでいません。
...
- diagnostic log schemaの必須keyを独立したschema/check関数として固定すること。
...
次は正式DB diagnostic log outputの構造固定、またはdiagnostic logと将来の解析ログ/低信頼度ログ/source capture参照の分離を進めてください。本番insertはまだ実装しないでください。

第一候補:

- diagnostic log recordの必須keyを小さな関数またはテストで固定する。
- `log_schema_version`、`event_type`、`mode`、`format`、`exit_code`、`status`、`db_path`、`diagnostic_output_path`、`diagnostic` の必須性をテストする。
- `status=compatible` / `status=rejected` と `diagnostic.is_compatible` / CLI exit code の一致条件を固定する。
- JSONL append時に複数行がすべて1行1JSONとして読めること、空行を出さないことをテストする。
- README/docsで、diagnostic log outputは「DB診断ログ」であり、低信頼度ログ本番保存やsource capture保存とは別物として読むことをさらに明確化する。
...
- diagnostic log recordの必須keyをコード定数にするか、テスト上の期待dictに留めるか。
- `diagnostic_output_path` は未指定時に空文字でよいか、null相当へ変えるか。
- prepare-writeで新規DBを初期化した場合、log側では最終diagnosticだけで十分か、初期/最終statusをトップレベルにも出すか。
...
- `python -m pytest tests\test_personal_score_db_schema.py`: 52 passed
...
補足: `tests\test_vision_poc_ocr.py -k "m8"` 実行時に、pytestの一時ディレクトリ削除に関するWindows警告が1件出たが、テスト結果は成功。
