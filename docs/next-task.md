...
- M7a `score_digits`、`max_combo`、`marvelous`、`perfect`、`great`、`good`、`miss`、`ex_score` の縦持ち認識結果に加えて、confirmed-events 保存候補1件を1行にする横持ち集約を追加した。
- 追加出力:
  - `m7a_digit_save_candidate_summary.csv`
  - `m7a_digit_save_candidate_summary.json`
  - `m7a_digit_save_candidate_summary.md`
- 集約CSVは、選択ROIごとに `recognized_digits`、`expected_value`、`status`、`failure_reason`、`match`、`confidence`、`distance`、`segment_count` を持つ。
- 集約 `aggregate_status` は `all_digits_recognized`、`needs_digit_review`、`no_digit_rois` のみ。これは保存OK/NG判定ではなく、M8へ渡す数字読み取り材料。
- duplicate、`event_type=rejected_transition`、未確定候補、non-result は M7a集約対象外のまま維持している。
- fixtureでは、confirmed-events 1件だけが集約されること、`missing_reference`、`not_evaluated`、未試行の `no_digit_attempt` を混同しないことを追加確認した。
- ドキュメント更新済み:
...
- 2026-07-08のローカル `score_digits + max_combo + marvelous + perfect + great + good + miss + ex_score` 実行結果:
  ...
  - `match_count=480`
  - `mismatch_count=0`
  - `m7a_digit_save_candidate_summary.json` は `target_count=60`、`aggregate_status_counts={"all_digits_recognized":60}`。
...
- 2026-07-08の共有 `judgment_counts` root確認:
  ...
  - `match_count=422`
  - `mismatch_count=0`
  - `miss` は `status_counts={"ambiguous":58,"recognized":2}`、`failure_reason_counts={"distance_above_threshold":54,"low_margin":4}` の既知状態を維持。
...
M7a横持ち集約の次ステップとして、`m7a_digit_save_candidate_summary.*` から「レビューすべき数字ROI」を代表化する小さなレポートを追加する。

- 第一候補は `m7a_digit_save_candidate_review.json` / `.md` のような代表レポート。
- 入力は既存の `m7a_digit_save_candidate_summary_rows` の行だけに限定する。
- confirmed-events 境界は変えない。duplicate、`rejected_transition`、未確定候補、non-result は対象外のままにする。
- `aggregate_status=needs_digit_review` の行を、ROI別 `status` / `failure_reason` ごとに代表化する。
...
- `all_digits_recognized` の代表は出してもよいが、主目的は `ambiguous`、`missing_reference`、`failed_segmentation`、`not_evaluated` のレビュー補助にする。
- `missing_reference` はテンプレート不足、`ambiguous` は距離/余白不足、`failed_segmentation` は桁分割失敗、`not_evaluated` は期待値不足または未試行として読み分ける。
- 保存OK/NG判定、DB保存、曲ID/譜面ID確定には進まない。
- ローカルテンプレートあり環境では8 ROI 480/480 matchと `aggregate_status_counts={"all_digits_recognized":60}` を維持する。
- 共有 `judgment_counts` root では `miss` の `ambiguous=58` と `needs_digit_review=58` を代表化できることを確認する。
...
- 次に追加するレビュー代表レポートも、保存OK/NG判定やDB保存に進んでいない。
