...
app-owned runtimeのlive observationは、上記の根拠をfield別に明示したformal evidenceとして保存bridgeへ渡せる。bridgeはmaster version、`RESULT同定根拠`、`RESULT数値認識根拠`、`RESULT状態認識根拠`についてsourceが契約値と一致し、confidenceが0.98以上かつ1.0以下であること、全必須値が揃うことを再検査する。play_idとplayed_atはconfirmed capture eventのID・UTC、duplicate keyはcapture event metadataから構築する。値または根拠が不足、sourceが未採用、confidenceが不足、digit statusが`ambiguous` / `missing_reference` / `failed_segmentation`の場合は `formal_evidence.*` または `digit_recognition.*` の理由を返し、既存formal workflowへplayなしで渡す。`identity_signal_*`、`recognized_digits`、candidate material、expected値、preview、`known-result`、OCRはformal evidenceのfallbackにしない。live source captureは `source_kind=capture` とし、capture hashはevent IDとPNG bytesを含むevent-scoped hashとする。
現行の標準`AppOwnedLiveResultAnalyzer`は`RESULT数値認識根拠`のcandidate-only observationを生成するため、上流のapp-owned producerが要件レベルのformal evidenceを付与しない既定live経路は意図的に`unresolved`となる。`RESULT同定根拠`、`RESULT状態認識根拠`をcandidate値、OCR、`known-result`から補完しない。
