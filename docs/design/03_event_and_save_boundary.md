...
app-owned runtimeのlive observationは、上記の根拠をfield別に明示したformal evidenceとして保存bridgeへ渡す。RESULT同定はcurrent master/catalogのread-only参照、ジャケット画像照合、譜面条件の色・数字画像認識を一意に通過した場合だけ採用し、OCRや候補値を使わない。bridgeはmaster version、`RESULT同定根拠`、`RESULT数値認識根拠`、`RESULT状態認識根拠`についてsourceが契約値と一致し、confidenceが0.98以上かつ1.0以下であること、全必須値が揃うことを再検査する。clear typeの内部判定に必要な`O.K.`数も画像認識根拠を要求するが、正式DB schemaへは保存しない。play_idとplayed_atはconfirmed capture eventのID・UTC、duplicate keyはevent boundaryで一度だけ発行したconfirmed event IDから構築する。同一RESULT画面の後続frame、再送、再処理は同じIDを使い、別confirmed eventでは正式値が同じでも新しいIDを使う。曲・譜面・スコア・判定数・rank・clear typeのRESULT fingerprintはイベントグルーピング専用で、formal duplicate keyには使わない。値または根拠が不足、sourceが未採用、confidenceが不足、digit statusが`ambiguous` / `missing_reference` / `failed_segmentation`の場合は `formal_evidence.*` または `digit_recognition.*` の理由を返し、既存formal workflowへplayなしで渡す。`identity_signal_*`、`recognized_digits`、candidate material、expected値、preview、`known-result`、OCRはformal evidenceのfallbackにしない。live source captureは `source_kind=capture` とし、capture hashはcapture event IDとPNG bytesを含むevent-scoped hashとする。
