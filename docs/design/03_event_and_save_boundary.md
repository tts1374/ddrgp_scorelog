...
M7保存判定プレビューでは、`m7_save_decision_preview.json` / Markdown の `preview_status_counts`、`preview_candidate_count`、status別代表を見て、M8へ渡す候補材料が揃った行と、M3 readiness、M7a数字レビュー、M5 identity review、必須材料欠落で止まる行を分けて読む。`preview_save_candidate` でも保存OKやDB保存成功ではなく、`identity_signal_*` とM7a数字列は候補観測のまま扱う。
