...
自動formal根拠は候補値の存在だけでは足りない。`RESULT同定根拠`、`RESULT数値認識根拠`、`RESULT状態認識根拠`、`capture event根拠`ごとにsourceと最低confidenceを要求する。played_atはcapture UTC、master versionはmaster metadata、song/chart identityは`result_identity_visual_evidence`、8つの数字fieldは`result_numeric_visual_evidence`、rankは`result_rank_visual_evidence`、clear typeは`result_clear_type_visual_evidence`から接続する。formal scoreは0〜1,000,000の10刻みで検証する。RESULTのrank ROIはFAILEDを示すE判定だけに使い、Eならrank=`E` / clear_type=`FAILED`とする。非FAILEDのrankはformal scoreの閾値表から算出し、clear_typeは認識済み判定数から算出する。rank周囲のanimation表示やexpected値、`match`、M8 preview、raw OCR、相対時刻はsourceにできない。flare_rankはrankから独立したbadge ROIで認識できた場合だけ採用し、認識不能時は `null` として保存継続し、no-flareの証明にはしない。現行解析でrank/clear type等の採用済み根拠がないeventは `unresolved` であり、play rowを作らない。

app-owned runtimeのlive observationは、上記の根拠をfield別に明示したformal evidenceとして保存bridgeへ渡す。RESULT同定はcurrent master/catalogのread-only参照、ジャケット画像照合、譜面条件の色・数字画像認識を一意に通過した場合だけ採用し、OCRや候補値を使わない。bridgeはmaster version、`RESULT同定根拠`、`RESULT数値認識根拠`、`RESULT状態認識根拠`についてsourceが契約値と一致し、confidenceが0.98以上かつ1.0以下であること、全必須値が揃うことを再検査する。clear typeの内部判定に必要な`O.K.`数は従来どおり画像認識根拠を要求し、その認識済み値を正式playのnullableな`ok`へ接続する。消費カロリーはapp-owned画像認識で採用できた場合だけnullableな`calories`へ接続し、未取得・低信頼度・認識失敗だけを理由に他の正式保存条件を満たすplayを止めない。どちらも未取得値をcandidate、expected、preview、raw OCR、0で補完しない。play_idとplayed_atはconfirmed capture eventのID・UTC、duplicate keyはevent boundaryで一度だけ発行したconfirmed event IDから構築する。同一RESULT画面の後続frame、再送、再処理は同じIDを使い、別confirmed eventでは正式値が同じでも新しいIDを使う。曲・譜面・スコア・判定数・rank・clear typeのRESULT fingerprintはイベントグルーピング専用で、formal duplicate keyには使わない。必須値または必須根拠が不足、sourceが未採用、confidenceが不足、必須digit statusが`ambiguous` / `missing_reference` / `failed_segmentation`の場合は `formal_evidence.*` または `digit_recognition.*` の理由を返し、既存formal workflowへplayなしで渡す。`identity_signal_*`、`recognized_digits`、candidate material、expected値、preview、`known-result`、OCRはformal evidenceのfallbackにしない。live source captureは `source_kind=capture` とし、capture hashはcapture event IDとPNG bytesを含むevent-scoped hashとする。

screenshot importは、拡張子・PNG decode・1280x720を通過した元画像を通常のRESULT screen detectionから同じapp-owned recognitionとformal evidence bridgeへ渡す。1 PNGを1独立処理・1 transactionとし、`source_kind=manual`、元PNGの絶対path、最終更新UTCをsource/played timeへ使う。duplicate keyだけはcapture event IDの代わりにPNG bytesのSHA-256から`screenshot-import-v1:<sha256>`を決定し、同一bytesの保存済みplayだけをduplicateとする。capture ID、capture hash、analysis IDは試行単位で一意にし、unresolvedでDB未書込のPNGは再試行可能とする。live capture keyとの類似推定や画像近似duplicateは行わない。

live monitoringとscreenshot importの解析は並行可能とし、同一app processの正式DB writer呼出しだけを共有gateで直列化する。gateは1回のwriter transaction直前に取得して完了後に解放し、batch全体では保持しない。cancelまたはapplication exitは新しい画像処理を開始せず、開始済みtransactionのcommit/rollbackを待つ。保存playが1件以上あるbatchだけ終了後にviewerを1回再読込し、その時点の正式DB全体を表示する。
...
画像照合がjacket一意の場合は従来のjacket判定だけを使い、M7 result-text featureを実行しない。jacketがambiguousの場合だけ、chart候補集合とjacket曖昧候補song ID集合の共通部分に`song_title`の画像featureを比較し、通常featureがambiguousのときだけ`title_linehash_rows`で再順位付けし、解消しないときだけ同じ候補集合で`artist`を比較する。比較距離は正規化距離`0.35`以下を採用条件とし、これを超える最良候補はmarginに関係なく未解決とする。`m7-result-text-image-v1`、`m7-result-title-artist-roi-v1`、payload/hash、current masterのcanonical title/artistがすべて一致し、現行flat shape `[1536]` / `[640]` を持つ一意候補だけをformal identityへ接続する。nested shape `[96,16]` / `[40,16]` はリリース前の現行形式外として読み込み対象外とする。候補集合外のsongを追加検索・選択しない。featureが欠落、壊れている、旧version、current masterと不整合、confidence不足、または複数候補のままなら`unresolved`であり、候補値や補助featureをformal identityへ昇格しない。この補助比較は既存のjacket threshold/ambiguity delta、event boundary、confirmed event ID、RESULT fingerprint、duplicate key、formal evidence bridge、transaction、正式保存workflowを変更せず、DB insertを追加で発行しない。
...
capture-save由来の `capture_hash` は `capture_id` と画像bytesを入力とするcapture-event version 1 hashとする。同じmanifest/frameの再実行は同一hashとなり、byte-identicalでも別frame indexは別hashとなる。同一表示中の静止リザルトの後続サンプルはRESULT fingerprintで1つのeventへ集約し、non-result等でevent boundaryを抜けてから確定した別候補だけを別eventとして扱う。confirmed event IDはこのevent boundaryで一度だけ発行し、正式 `duplicate_key` に使う。その別eventが正式 `duplicate_key` と衝突した場合にだけ、playなしのduplicate source capture + analysisとして記録し、同一capture event再送のUNIQUE拒否も維持する。live監視のcandidateはPNGとmanifestをOS一時directoryへ置いてこのhashを計算するが、workflow完了後に一時directoryを削除する。live由来の `source_captures` は `source_path=live-memory://...` と空の `manifest_image_path` を記録し、画像の永続参照は残さない。
