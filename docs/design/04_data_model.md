...
保存前validation結果はDB modelではなくCLI境界の投影である。`validation_result_schema_version`、入力path、`adapter_status`、`save_input_constructed`、理由だけを持ち、`PersonalScoreDbSaveInput` の正式値や `candidate_material` を複製しない。`save_input_constructed=true` はadapterがready/excludedの正式入力を組み立てたことだけを表し、DB row作成、DB互換性、duplicate非衝突を意味しない。
