...
詳細はversion 1 JSONとしてDB外に置き、`log_path` は `logs/analysis_details/` 基準の相対 `.json` pathだけを参照する。空文字は既存互換として許可する。DB診断JSONL、validation receipt、source capture、失敗画像を指さない。

詳細JSONはschema/generator、analysis/source ID、保存境界status、event確定情報、identity/digit review、confidence、再調査用候補summary、任意の `failure_image_path`、retention metadataを持つ。正式 `plays` 値、validation receipt projection、DB diagnostic payloadは複製しない。失敗画像は `logs/analysis_failures/` の別相対pathであり、元フレームを指す `source_captures.source_path` とも異なる。
...
`played_at` / `captured_at` はtimezone付きISO 8601、`rank` / `clear_type` は空文字不可とする。`flare_rank` は `I`〜`IX` / `EX` または `null`、`ok`は非負整数または`null`、`calories`は非負の有限小数または`null`を許可する。これらの任意値の認識不能だけを理由に保存全体を止めない。`ok=null`でもclear type等の既存必須条件は緩和せず、両値ともcandidate/raw/expected/preview/0から補完しない。timestampなしpreviewの `played_at_ms=0`、PoCの `score:` / `file:` duplicate key、由来不明の `source_kind=unknown` は正式writer入力として拒否する。

`tools.vision_poc.personal_score_db_save_adapter` はこの直前のpure境界を担当する。M8 payload/planned rowを `candidate_material` として受け取っても、その候補ID、候補数字、相対時刻、`ok`、`calories`は `PersonalScoreDbFormalPlayValues` へ転記しない。レビュー済み正式値にrank、clear type、任意のflare_rank/ok/caloriesを明示し、その他の必須正式値も揃った場合だけ `ready` として `PersonalScoreDbSaveInput` を返す。任意値を認識できない場合は `null` のまま扱い、candidate/raw/expected/previewから補完しない。レビュー済み必須正式値が不足または正式入力検査に失敗する場合は `unresolved` として `PersonalScoreDbSaveInput` を返さない。duplicateまたは明示された低信頼度/error/skipは `excluded` とし、`plays` 値を持たないsource capture + analysis入力だけを返す。
...
保存前validation結果と、その同一投影を明示的に保存するreceiptはDB modelではなくCLI境界のレビュー記録である。`validation_result_schema_version`、入力path、`adapter_status`、`save_input_constructed`、理由だけを持ち、`PersonalScoreDbSaveInput` の正式値や `candidate_material` を複製しない。`save_input_constructed=true` はadapterがready/excludedの正式入力を組み立てたことだけを表し、レビュー承認、DB row作成、DB互換性、duplicate非衝突を意味しない。

review templateもDB modelや候補payloadではなく、`input_schema_version=1` の入力構造を人に提示するための空objectである。`candidate_material={}`、全fieldを明示した未確定 `formal_play`、`exclusion=null` を持つが、正式値や候補値を生成しない。未編集状態から `PersonalScoreDbSaveInput` は構築できず、`unresolved` を固定する。
