...
- `user settings`: WPF appの起動時監視、保存できない結果のローカル通知、既定プレイスタイル、起動時画面、自己ベストの探索モード・レベル・バージョンを保持する正式DB外のローカル設定。`user-settings.json`へ保存し、欠落・読込不能時は全項目を初期値へ戻す。正式個人スコアDB、`plays`、保存境界は変更しない。
