...
公式リストとWiki譜面マスタの突合は、まず曲名+artistの正規化一致で行い、artistが空または表記差がある場合は曲名が公式リスト内で一意な場合だけ曲名一致で補完する。突合結果は `official_availability_match` に残す。公式リストにない曲や曖昧な曲は `grand_prix_play_available=false` のままにし、M5の通常候補から除外する。
...
- `official_availability_match`: 公式収録曲一覧との突合状態。`title_artist` / `unique_title` / `ambiguous_title` / `not_found` / `not_checked`。
- `notes`
- `created_at`
- `updated_at`
