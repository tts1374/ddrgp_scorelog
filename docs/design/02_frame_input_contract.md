...
明示pickerで選んだ1つのwindowを、明示停止まで同じcapture item、D3D11 device、free-threaded frame pool、capture sessionで取得する。出力は次のmanifest互換bundleとする。
...
各frameは取得順に連番を付け、`timestamp_ms` はprocess単調時刻を基に同値時だけ直前値 + 1msへ補正してstrictly increasingにする。`image_path` はmanifest directory相対の `frames/frame-*.png` とし、`screen_type=unknown`、`capture_source`、`width`、`height`、`captured_at_utc` を任意列として保持する。bounded frame queueが満杯のときは中間frameをdropし、resource保持量を制限するが、保存した行の時系列とtimestamp単調性は維持する。

明示停止かつ1frame以上を保存できた場合だけmanifestとsession metadataを完成させ、`data/` 直下のstaging directoryから一意な最終directoryへrenameする。0frame停止、cancel、access拒否、target closed、resize、device lost、write失敗ではstagingを削除し、成功sessionへ丸めない。resizeはsingle-frameの `Resized` 契約を継承して自動追従せず停止し、再選択を求める。停止、失敗、window closeではevent購読を解除し、queued frame、capture session、frame pool、D3D11 deviceを一度だけ解放する。
...
- WPF continuous capture sessionが出す複数行manifestもmanifest directory相対で読め、capture補助列とstrictly increasingなtimestampを維持する。
