...
- `miss` はROI左側ラベル、短いマーカー、明るい青背景を数字として数えず、他の判定数と同じforeground maskで右側数字領域のコンポーネントを分割する。
...
- 判定数系テンプレート（`marvelous`、`perfect`、`great`、`good`、`miss`）は共有 `judgment_counts` ディレクトリから読む。
...
| 保存前identity照合 | current checkpoint、全review状態のcurrent catalog identity集合、明示opt-in | checkpoint/catalog既存identityは新規artifact/checkpointを作らず、自動保存はsession単位・既定OFF・identityごとに1回だけ試行する |
...
### Production auto-confirm / manual ODS guard

- PR #53の3経路だけをauto対象とし、policy判定をwriter側へ複製しない。
- dry-runを既定にし、`--apply`とpreflight planの明示なしではcatalogを書かない。
- apply時はpolicy target/evidenceを再構築し、plan自己hashだけを登録根拠にしない。
- ODS、Master、snapshot/policy revision、catalog logical guard、対象row stateを検査し、driftを全件拒否する。
- 全auto targetを1つの`BEGIN IMMEDIATE` transactionで処理し、途中例外とcommit前external driftで全件rollbackする。
- exact evidenceの同一plan再applyはno-opとし、重複observation、不正song、既存manual/rejected/別根拠確定は拒否する。
- confirmation source、matched song、policy/snapshot/feature、distance/margin/rank、OCR profile/raw/normalizedをversion付きevidence JSONとして保持する。
- song IDを正本とし、canonical title/artistはcurrent Masterから取得する。manual action/historyをauto-confirmへ流用しない。
- manual ODSはread-only exportに限定し、21 manualだけを含め、7 capture mismatchを含めない。
- 同じplanからのODS再exportをbyte-stableにし、既存fileを上書きせず、catalog/master/review stateを変更しない。
- current manual review XLSXはprojectionの`needs_review` / `unresolved`だけを対象に、title/artist ROIをXLSX packageへ埋め込み、Manual Review・Master Songs・Metadataの3 sheetを持つ。対象0件でもheaderとMetadata対象件数0を作り、Manual Reviewの`status`には固定4値の入力規則を付ける。
- manual review XLSXは呼び出し側が明示した`.xlsx`へ出力し、画像を検証できない対象は省略せず全体を拒否する。WPFでは標準保存ダイアログの確認後に既存fileを置き換える。catalog、Master、下書き、source/crop画像を変更しない。
- manual review XLSX importはManual Review・Master Songs・Metadataの存在とschema/column、current projection上のobservation ID、status/song制約、Metadata対象件数を全件検証する。1行でも不正なら行番号・observation ID・理由を返し、メモリ上・保存済み下書きを変更しない。成功時だけ既存draft storeへ一括保存して画面へ再適用し、catalog、Master、review transaction、history、確定状態を変更しない。catalog/Master version差だけでは拒否せず、レビュー済み対象、画像、ODSはimportしない。
- plan、検証copy DB、snapshot、画像、reportを`data/`外またはGitへ出さない。manual review XLSXは利用者が選択した保存先へ出力できる。
- collectorの明示的な`収集を終了`は、`pending` retry完了、current projection matching、既存#53 `ocr_title_artist_pair` targetの一括transaction、projection再読込の順で実行する。pending残留、auto transaction失敗、current master/catalog/artifact/checkpoint driftではauto-confirmを成功扱いにしない。
- collectorのunsafe stop、通常catalog retry、manual review、coverage、候補reportはauto-confirmを起動しない。auto-confirm済み行は既存WPF manual reviewの未レビュー一覧から除外し、残件数は同じpersisted status境界で表示する。
