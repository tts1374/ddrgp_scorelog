...
- current manual review ODSはprojectionの`needs_review` / `unresolved`だけを対象に、title/artist ROIをODS packageへ埋め込み、Manual Review・Master Songs・Metadataの3 sheetを持つ。対象0件でもheaderとMetadata対象件数0を作る。
- manual review ODSの出力先は`data/`配下の新規fileに限定し、画像を検証できない対象は省略せず全体を拒否する。catalog、Master、下書き、source/crop画像を変更しない。
- plan、ODS、検証copy DB、snapshot、画像、reportを`data/`外またはGitへ出さない。
