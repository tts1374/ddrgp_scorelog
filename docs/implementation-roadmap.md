...
- 2026-07-08時点のローカル `miss` ROI別テンプレート配置後は、`score_digits`、`max_combo`、`marvelous`、`perfect`、`great`、`good`、`miss` の7 ROIで confirmed-events 60件ずつ、合計420/420 `recognized` / match。`miss` は短いマーカーや幅広い背景成分を数字扱いしないよう、右側数字領域へのfocus、最小高さ、幅/高さ比をROI別に絞っている。共有 `judgment_counts` だけでは `miss` が60/60 `ambiguous` になるため、ROI別テンプレートを使う。
