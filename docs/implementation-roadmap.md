...
- 第4段階以降として、実capture認識品質、連続capture、正式保存接続、監視状態を順に接続する。
- タスクトレイ常駐を実装する。
...
1. `M9残り実行順` の2項目目として、実capture manifestを再実行し、曲・譜面同定と数字認識を実capture投入可能な品質へ上げる。
2. 以降は同実行順の3から6に従い、連続capture、正式保存接続、監視UI・タスクトレイ、長時間運用を1項目ずつ進める。
