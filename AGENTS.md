...
- 主実装後も、必要なテスト、README・設計docs同期、検証、diff確認、下記Review Policyの独立review gateまで続行する。
...
- 次の責務を複数まとめて大きく変更しない。
  ...
  - UI binding
  - PythonとC#間のエラー契約
- 複数の状態機械、永続化境界、言語間契約を同時に変更する場合は、まず独立してmerge可能なPRへ分割する。
- 受入条件や境界条件を簡潔に説明できないPR、または複数の独立した失敗経路を持つPRは分割対象とする。固定的なファイル数や行数だけでは機械的に判定しない。
- PRが大きいためTeamが必要に見える場合は、Team投入より先にPR分割を検討する。
...
- push後に、対応内容、検証結果、commit SHAを記載したtop-level PR commentを1件投稿し、Review Policyの回数上限内で必要な場合だけ末尾で `@codex review` を依頼すること
...
GitHub Codex Reviewの回数、原因クラス監査、2回目後の収束判断は、下記Review Policyを唯一の事実源とする。
...
- 通常の小規模PRは `GPT-5.6 Sol / medium` を基準とする。
- 小規模でも、保存境界、schema、transaction、duplicate、checkpoint、concurrencyなど高リスクな責務を変更するPRは `GPT-5.6 Sol / high` を基準とする。高リスクであることだけを理由にTeamを利用しない。
- `GPT-5.6 Terra` は、代表的な作業で必要品質を維持できる場合の性能とコストのバランス候補とする。
- `xhigh` 以上は、未確定仕様の裁定、重大な設計矛盾、難しい原因調査、最終監査に限定する。通常実装や高リスクであることだけを理由に引き上げない。
...
### Execution Mode And Team Opt-in

- 通常タスクはLead-onlyで実行する。単一責務で範囲が小さい変更は、schema、transaction、duplicate、checkpoint、concurrencyなどを含んでいてもLead-onlyとする。
- `$codex-model-routing-team` は、次をすべて満たす場合だけopt-inで利用する。
  - 独立して実装可能なworkstreamが2つ以上ある。
  - 各workstreamの仕様、境界条件、受入条件が固定済みである。
  - file ownershipを重複なく分割できる。
  - worker間の調整や統合コストより並列化効果が大きい。
  - PRを分割するより、同時にmerge・revertする方が自然である。
- 高リスク、PRが大きい、独立レビューが必要、実装担当とは別のmodelに確認させたい、という理由だけではTeamを利用しない。大きなPRは先にPR Split Policyを適用する。
- Teamを利用すると決めた後のmodel routing、worker lifecycle、task packet、隔離worktree、所有権、統合、archive、報告は `$codex-model-routing-team` を唯一の事実源とし、詳細手順をここへ複製しない。
- Team利用の有無にかかわらず、push、PR作成、GitHub comment、外部サービス書込み、merge、release、削除、migration実行はleadだけが既存の許可境界内で行う。
...
- レビューと限定監査では、変更責務に該当する正常系、空/欠損、重複、競合、再投入、旧version、部分失敗、option混在を境界条件マトリクスとして確認する。該当しない軸は理由を示して除外してよい。
- schema、field、status、option、永続化形式を変更した場合は、書く側だけでなく、それを読む全consumer、集計、再読込、互換経路を検索し、旧値・旧versionを現行値として誤解しないことを確認する。
- validation、duplicate、skip、互換性拒否、option不整合などのearly returnを追加・変更した場合は、returnより前に起こり得る入力読込、directory/file作成、DB準備・書込み、log/artifact出力を列挙し、拒否時に副作用がないtestを置く。
